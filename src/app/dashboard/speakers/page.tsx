...
const ChatListItem = ({
  ...
  <button
    className={`w-full flex items-center p-2 sm:p-3 rounded-2xl shadow-lg bg-gradient-to-r ${colorClass} transition-transform hover:scale-[1.02]`}
  >
...
const SpeakersPageV2: NextPage = () => {
  ...
  const { savedSpeakers, loading: speakersLoading } = useSpeakers();
  ...
  const recentChats = conversations.map((conversation, index) => ({
    ...
    colorClass: getGradientClass(index),
    avatarUrl:
      conversation.speakerAvatar ||
      `https://api.dicebear.com/9.x/avataaars/svg?seed=${conversation.speakerName}`,
    flagUrl:
      conversation.flagUrl || "https://unpkg.com/circle-flags/flags/us.svg",
  }));

  const onSpeakerCatalogClick = useCallback(
    () => alert("Open speaker catalog!"),
    []
  );
        ...
        <section className="flex flex-col items-start gap-4 mt-8 col-start-2 w-full">
          ...
          {chatsLoading ? (
            ...
            <div className="w-full flex flex-col gap-3">
              {recentChats.map((chat: ChatListItemProps) => (
