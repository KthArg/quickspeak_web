...
const SignUpPage: NextPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setServerError(err?.message || "Network/API error");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        ...
        {serverError && (
          <div className="w-full">
            <p className="text-red-500 font-bold ml-1">{serverError}</p>
          </div>
        )}
        ...
        <div className="w-full flex flex-col items-center justify-center gap-3 text-lg mt-3">
          <button
            ...
            className={`w-full rounded-full flex items-center justify-center py-3 px-5 gap-2.5 transition-colors font-extrabold text-xl ${
              isDark
                ? "bg-[#18D2B4] text-[#073b4c] hover:bg-[#14a892] disabled:opacity-70"
                : "bg-teal-400 text-white hover:bg-teal-500 disabled:opacity-70"
            }`}
          >
            <b>{submitting ? "Creating..." : "Sign Up"}</b>
