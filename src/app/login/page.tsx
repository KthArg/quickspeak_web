      ...
      <form onSubmit={handleSubmit} noValidate className="relative z-10 w-full max-w-sm flex flex-col items-start gap-5">
        ...
        <div className="w-full flex flex-col items-center justify-center gap-3 text-lg mt-3">
          ...
          <div className={`text-center text-base ${isDark ? 'text-gray-300' : 'text-gray-800'}`}>
            <b>No account? <a href="/sign_up" className={`underline cursor-pointer ${isDark ? 'text-[#18D2B4] hover:text-white' : 'text-black hover:text-purple-600'}`}>Sign Up</a></b>
          </div>
        </div>
        ...
        <div className="w-full flex flex-col items-start gap-4">
          <div className="w-full flex items-center gap-4">
            ...
            <div className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-500'}`}><b>Or Log in With</b></div>
            <div className={`flex-1 rounded-full h-0.5 ${isDark ? 'bg-gray-400' : 'bg-gray-300'}`}></div>
          </div>
          <div className="w-full flex flex-col sm:flex-row justify-center items-center gap-4">
             <a href='/.auth/login/google?post_login_redirect_uri=/dashboard/speakers' type="button" className={`w-full sm:w-auto flex-1 h-16 rounded-full flex justify-center items-center transition-colors ring-4 ring-offset-2 
