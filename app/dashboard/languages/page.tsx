    ...
    return (
        <button onClick={onClick} className="flex flex-col items-center gap-2 text-center transition-transform hover:scale-105">
            ...
            <div className="flex flex-col">
                <span className={`font-bold text-lg ${theme === 'dark' ? 'text-white' : 'text-black'}`}>{lang.name}</span>
...
const AddLanguageCard = () => {
    const { theme } = useTheme();
    return (
        <button className={`w-full max-w-sm sm:max-w-md flex items-center p-3 sm:p-4 rounded-2xl shadow-lg transition-transform hover:scale-[1.02] ${theme === 'dark' ? 'bg-cyan-500/80' : 'bg-cyan-400'}`}>
            <div className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-full flex-shrink-0 flex items-center justify-center ${theme === 'dark' ? 'bg-gray-700/50' : 'bg-white/50'}`}>
                <Languages size={40} className={`${theme === 'dark' ? 'text-cyan-200' : 'text-cyan-800'}`} />
            </div>
            <div className="flex-grow flex flex-col items-start ml-4 text-left">
                <h3 className={`font-bold text-xl sm:text-2xl ${theme === 'dark' ? 'text-black' : 'text-white'}`}>More Languages</h3>
                <p className={`font-semibold text-sm sm:text-md ${theme === 'dark' ? 'text-gray-800' : 'text-gray-100'}`}>Tap here to add another language</p>
            </div>
        </button>
    );
};
    ...
    return (
        <div onClick={onClose} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div onClick={(e) => e.stopPropagation()} 
                ...
                ${theme === 'dark' ? 'bg-gray-800 border-2 border-cyan-400/50' : 'bg-white'}`}
            >
                <button onClick={onClose} className={`absolute top-3 right-3 flex items-center gap-1.5 px-3 py-1 border-2 rounded-full text-sm font-semibold 
                    ${theme === 'dark' ? 'border-cyan-400/50 text-cyan-400 hover:bg-cyan-400/10' : 'border-gray-400 text-gray-600 hover:bg-gray-100'}`}
                >
                    <X size={16} /> Close
                </button>
                ...
                <h2 className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-black'}`}>{lang.name}</h2>
                ...
                <div className="w-full flex flex-col items-center gap-3 text-center">
                    <button 
                        ...
                        className={`w-full rounded-full py-2.5 font-bold text-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors
                            ${theme === 'dark' ? 'bg-cyan-400 text-black' : 'bg-cyan-500 text-white'}`}
                    >
                        {isNative ? 'Current Native Language' : 'Make Native Language'}
                    </button>
                    <p className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} max-w-xs`}>
                        This will make this the language in which clarifications are given to you.
                    </p>
                </div>
                ...
                {!isNative && (
                    <button 
                        ...
                        className={`w-full rounded-full py-2 font-bold text-lg transition-colors
                            ${theme === 'dark' ? 'bg-transparent border-2 border-red-500 text-red-500 hover:bg-red-500/10' : 'bg-white border-2 border-red-500 text-red-500 hover:bg-red-500/5'}`}
                    >
...
const LanguagesPage: NextPage = () => {
  ...
  const [nativeLanguageId, setNativeLanguageId] = useState(1);
  ...
  return (
    <div className={`w-full min-h-screen relative font-cabin flex flex-col items-center p-6 md:p-10 transition-colors
        ...
        <header className="text-center mb-10 sm:mb-16">
            <h1 className={`text-6xl sm:text-7xl md:text-8xl font-bold ${theme === 'dark' ? 'text-cyan-400' : 'text-cyan-500'}`}>Languages</h1>
        </header>
